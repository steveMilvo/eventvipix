...
export default function QRGallery() {
  ...
  const qrCodes: QRCodeItem[] = [
    {
      ...
      packageType: "premium",
      status: "active",
      folderPath: "/events/wedding-sarah-john",
      cameraLink: "/camera/WEDDING1",
      photoCount: 0
    },
    {
      id: 2,
      ...
      packageType: "premium",
      status: "active",
      folderPath: "/events/corporate-conference-2025",
      cameraLink: "/camera/CONF2025",
      photoCount: 0
    },
    {
      id: 3,
      eventName: "Birthday Party: Mike's 30th",
      eventDate: "2025-05-30T20:00:00.000Z",
      loginCode: "BDAY30",
      qrCode: sampleQRCode,
      packageType: "standard",
      status: "active",
      folderPath: "/events/birthday-mike-30th",
      cameraLink: "/camera/BDAY30",
      photoCount: 0
    }
  ];
  ...
  const filteredQRCodes = qrCodes.filter(item =>
    ...
    item.loginCode.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const downloadQRCode = (item: QRCodeItem) => {
    ...
    link.download = `${item.eventName}-QR-Code.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const shareQRCode = async (item: QRCodeItem) => {
    if (navigator.share) {
      try {
        await navigator.share({
          title: `QR Code for ${item.eventName}`,
          text: `Access code: ${item.loginCode}`,
          url: window.location.origin + item.cameraLink
        });
      } catch (err) {
      ...
      navigator.clipboard.writeText(window.location.origin + item.cameraLink);
    }
  };
  ...
  return (
    <div className="container mx-auto py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">QR Code Gallery</h1>
          <p className="text-gray-600 mt-2">Manage and access all your event QR codes</p>
        </div>
          ...
          <Card key={item.id} className="hover:shadow-lg transition-shadow">
            <CardHeader className="pb-4">
              <div className="flex justify-between items-start">
                <CardTitle className="text-lg line-clamp-2">{item.eventName}</CardTitle>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-center">
                <div className="bg-white p-4 rounded-lg border-2 border-dashed border-gray-200">
                  <img 
                    src={item.qrCode} 
                    alt={`QR Code for ${item.eventName}`}
                    className="w-32 h-32"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="capitalize text-sm">{item.packageType}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm">{item.photoCount}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="text-xs text-gray-500 space-y-1">
                <div className="flex items-center gap-1">
                  ...
                  <span>Folder: {item.folderPath}</span>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">
            {searchTerm 
              ...
              : "QR codes will appear here after completing event registration"
            }
          </p>
        </div>
      )}
